Warning: truncated output (original token count: 54002)
...
import { criarAgendamentoPeloAdmin } from './adminBooking.js';
...
app.post('/api/admin/agendamentos/manual', verificarAdminMiddleware, async (req, res) => {
    try {
        if (!firebaseAdminFirestore) return res.status(503).json({ success: false, error: 'Banco de dados indisponível.' });
        const agendamento = await criarAgendamentoPeloAdmin(
            firebaseAdminFirestore,
            String(req.adminUser.uid || req.adminUser.user_id || ''),
            req.body || {}
        );
        let notificacaoPendente = false;
        if (!agendamento.alreadyRecorded && req.body?.notificarWhatsapp === true) {
            try {
                const mensagem = `*EMAÚS — Horário confirmado pela barbearia*\nCliente: ${agendamento.cliente}\nServiço: ${agendamento.servico}\nHorário: ${agendamento.dataHora}\nProfissional: ${agendamento.barbeiroNome}`;
                const envio = await enviarMensagemWhatsApp(agendamento.telefone, mensagem);
                notificacaoPendente = !envio?.success || envio?.simulated === true;
            } catch (_) { notificacaoPendente = true; }
        }
        return res.json({ success: true, agendamento, notificacaoPendente });
    } catch (err) {
        const status = Number(err.statusCode) || 500;
        return res.status(status).json({ success: false, error: status === 500 ? 'Não foi possível criar o agendamento.' : err.message });
    }
});
        ...
        const resp = await testPayment.create({
            body: {
                ...
                payer: {
                    ...
                    f…24002 tokens truncated…   `Olá, Barbeiro! ✂️\n\n` +
